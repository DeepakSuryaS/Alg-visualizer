var algoOptions = document.getElementsByClassName("algorithmOption");
var sortSpeed = document.getElementById("changeSpeed");
var sortSpeedValue = 1;


for (var i = 0; i < algoOptions.length; i++) {
  algoOptions[i].addEventListener("click", function() {
...
async function bubbleSort(delay) {
  ...
  if(isSorted){ 
    ...
    res.innerHTML = 'Bubble Sort Started...';
    result.appendChild(res);
    ...
    for (let j = 0; j < blocks.length - i - 1; j += 1) {
      ...
      blocks[j + 1].style.backgroundColor = "rebeccapurple";
    }
  
    blocks[blocks.length - i - 1].style.backgroundColor = "#ffd700";
    }
    
    blocks[0].style.backgroundColor = "#ffd700";
    ...
    for (var v = 0; v < algoOptions.length; v++) {
    algoOptions[v].className = algoOptions[v].className.replace("disableClick","");
    }
